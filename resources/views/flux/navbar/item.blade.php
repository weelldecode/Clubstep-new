...
$classes = Flux::classes()
    ...
    ->add(match ($accent) {
        true => [
            ...
            'data-current:text-(--color-accent-content) data-current:bg-accent/10 hover:data-current:text-(--color-accent-content) hover:bg-zinc-800/5 dark:hover:bg-white/10 hover:data-current:bg-(--hover-fill)',
...
<flux:button-or-link :attributes="$attributes->class($classes)" data-flux-navbar-items>
    ...
    <?php if ($slot->isNotEmpty()): ?>
        <div class="{{ $icon ? 'ms-3' : '' }} flex-1 text-sm font-semibold leading-none whitespace-nowrap [[data-nav-footer]_&]:hidden [[data-nav-sidebar]_[data-nav-footer]_&]:block" data-content>{{ $slot }}</div>
    <?php endif; ?>
